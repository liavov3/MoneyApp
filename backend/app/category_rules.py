...
The raw `match_value` is consumed ONLY for matching here — never returned to the
client or logged (it is sensitive merchant text; API_CONTRACT §15).
"""
...
def resolve_suggestion(
    ...
    `source` is the contract enum: `<source>_<match_type>` for a rule (e.g.
    `user_correction_merchant_exact`), else `merchant_default` or `none`.
    """
    ...
    if default_category_id is not None:
        return default_category_id, default_category_key, "merchant_default"
...
if __name__ == "__main__":
    ...
    assert resolve_suggestion([], "golda", "c9", "transport") == ("c9", "transport", "merchant_default")
